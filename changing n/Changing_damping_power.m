...
g = 9.8;
omega_n = sqrt(g/l);
zeta = (1*(l^3))/(2*m*sqrt(g*l));

%Angular displacement for different damping power(n)
[t,theta] = ode113(@(t,theta) [theta(2); (-(omega_n^2).*sin(theta(1)))], [0, 15], [pi/3, 0]);
plot(t, theta(:, 1))
xlabel("time")
...
%Angular velocity for different damping power(n)
[t,theta] = ode113(@(t,theta) [theta(2); (-(omega_n^2).*sin(theta(1)))], [0, 15], [pi/3, 0]);
plot(t, theta(:, 2))
xlabel("time")
...
%Phase space plots for different damping power(n)
[t,theta] = ode113(@(t,theta) [theta(2); (-(omega_n^2).*sin(theta(1)))], [0, 15], [pi/3, 0]);
...
%Energy for different damping power(n)
Q_1 = [];
Q_2 = [];
Q_3 = [];
[t,theta] = ode113(@(t,theta) [theta(2); (-(omega_n^2).*sin(theta(1)))], [0, 15], [pi/3, 0]);
